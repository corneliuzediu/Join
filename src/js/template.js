...
function generateAssignedContactsHTML(contact, color) {
    return `<div style="background-color:${color}" class="task-contacts">${contact}</div>`;
};
...
function generateTaskProcessStatusforEditDialog(taskID) {
    ...
    if (currentTaskStatus === "to-do") {
        return (content.innerHTML = fromToDoMoveTo());
    } else if (currentTaskStatus === "await-feedback") {
        return (content.innerHTML = fromAwaitFeedbackMoveTo());
    } else if (currentTaskStatus === "in-progress") {
        return (content.innerHTML = fromInProgressMoveTo());
    } else {
        return (content.innerHTML = fromDoneMoveTo());
    }
}


function fromToDoMoveTo() {
    return `<option value="to-do" selected>To do</option>
...
function fromAwaitFeedbackMoveTo() {
    return `<option value="to-do">To do</option>
            <option value="in-progress">In progress</option>
...
function generateTaskModalHTML(task) {
    return `<div class="task-modal-container">
                <img class="close-icon-overlay" src="../img/add-task-close-icon.png" onclick="closeAddTaskDialog('task-modal', 'task-overlay')">
                ...
                <div class="current-process-status-bold" id="current-process-status-bold">
                    <span class="current-process-status" id="current-process-status"></span>
                </div>
                ...
                <div class="assigned-to-container">
                    ...
                    <div class="assigned-contacts" id="assigned-contacts${task["id"]}"></div>
                </div>
...
function generateEditTaskHTML(task) {
    return `<div class="task-modal-container">
                ...
                <form class="edit-task">
                    ...
                    <div class="uniform-sizing text-19pt dropdown" role="button" data-bs-toggle="collapse" data-bs-target="#collapseContactsEdit" aria-expanded="false" aria-controls="collapseContactsEdit" id="contact-dropdown-edit">
                        <span>Select contacts to assign</span>
                        <img src="../img/select-arrow.png" alt="">
                    </div>
                    <div class="assigned-contacts-initials" id="assigned-contacts"></div>
                    <div class="subtasks-input-area d-none" id="contact-input-area-edit">
                        ...
                        <div class="subtask-icons">
                            <img onclick="closeContactInput('contact-input-area-edit', 'contact-dropdown-edit', 'contact-input-edit')" class="cursor-pointer" src="../img/cancel-subtask.png" alt="">
                            <img src="../img/subtask-line.png" alt="">
                            <img onclick="addContact()" class="cursor-pointer" src="../img/check-subtask.png" alt="">
                        </div>
                    </div>
                    <div class="margin-btn-25 assign-contact-container" id="contact-container-edit">
                        <div class="dropdown-contacts-container collapse scroll" id="collapseContactsEdit">
                            <div class="dropdown-contact" onclick="openContactInput('contact-dropdown-edit', 'contact-input-area-edit', 'contact-input-edit')" role="button" data-bs-toggle="collapse" data-bs-target="#collapseContactsEdit" aria-expanded="false" aria-controls="collapseContactsEdit" id="contact-dropdown-edit">
...
function createSubtaskHTML(subtaskName) {
    return `<div class="subtask text-19pt">
...
function createSubtaskEditHTML(subtaskName, checkBox) {
    if (checkBox === true) {
        return `<div class="subtask text-19pt">
                    <label for="${subtaskName}" name="subtask-name" id="subtask-name" style="display: flex; gap: inherit;">
                        <input type="checkbox" id="${subtaskName}" checked="checked" name="subtask-checkbox">
                        ${subtaskName}
                    </label>
                </div>`;
    } else {
        return `<div class="subtask text-19pt">
                    <label for="${subtaskName}" name="subtask-name" id="subtask-name" style="display: flex; gap: inherit;">
                        <input type="checkbox" id="${subtaskName}" name="subtask-checkbox">
                        ${subtaskName}
                    </label>
                </div>`;
    };
...
function createContactHTML() {
    return `<div class="task-contacts-overlay-container">
